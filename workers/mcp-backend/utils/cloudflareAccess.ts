...
let _configProvider: (() => CloudflareAccessConfig) | null = null;

/**
 * Set the configuration provider for Cloudflare Access.
 * Call this once at startup to inject the config source.
 */
export function setCloudflareAccessConfigProvider(provider: () => CloudflareAccessConfig): void {
    _configProvider = provider;
}

/**
 * Get Cloudflare Access configuration.
 * Returns disabled config if no provider has been set.
 */
export function getCloudflareAccessConfig(): CloudflareAccessConfig {
    if (_configProvider) {
        return _configProvider();
    }
    return { enabled: false, teamDomain: '', policyAud: '' };
}
